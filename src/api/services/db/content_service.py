...
class ContentService:
    ...
    def get_by_session(cls, project_id: str, limit: Optional[int] = None) -> list[NovelContent]:
        ...
        with DatabaseSession() as db:
            ...
            logger.debug(f"获取会话小说内容: {project_id}, 共 {len(contents)} 行")
            return list(contents)
    
    @classmethod
    def get_by_chapter(cls, project_id: str, chapter: int) -> list[NovelContent]:
        """
        获取某个章节的所有内容。
        ...
        with DatabaseSession() as db:
            ...
            logger.debug(f"获取章节内容: {project_id} 第 {chapter} 章, 共 {len(contents)} 行")
            return list(contents)
    
    @classmethod
    def get_by_line(cls, project_id: str, chapter: int, line: int) -> Optional[NovelContent]:
        """
        获取某一行的内容。
        ...
        with DatabaseSession() as db:
            ...
            if content:
                db.expunge(content)
                logger.debug(f"获取行内容: {project_id} 第 {chapter} 章 第 {line} 行")
    ...
    def get_line_range(
        ...
        with DatabaseSession() as db:
            ...
            logger.debug(f"获取行范围内容: {project_id} 行 {start_line}-{end_line}, 共 {len(contents)} 行")
            return list(contents)
    
    @classmethod
    def count_by_session(cls, project_id: str) -> int:
        """
        统计某个 session 的总行数。
        ...
        with DatabaseSession() as db:
            result = db.exec(
                select(func.count()).select_from(NovelContent).where(
                    NovelContent.project_id == project_id
                )
            ).first()
            return result or 0
    ...
    @classmethod
    def count_chapters(cls, project_id: str) -> int:
        """
        统计某个 session 的总章节数。
        ...
        with DatabaseSession() as db:
            result = db.exec(
                select(func.count(func.distinct(NovelContent.chapter))).where(
                    NovelContent.project_id == project_id
                )
            ).first()
            return result or 0
    ...
    @classmethod
    def delete_by_session(cls, project_id: str) -> int:
        """
        ...
        with DatabaseSession() as db:
            ...
            logger.info(f"删除会话小说内容: {project_id}, 共 {count} 行")
            return count
    
    @classmethod
    def delete_by_chapter(cls, project_id: str, chapter: int) -> int:
        """
        删除某个章节的所有内容。
        ...
        with DatabaseSession() as db:
            ...
            logger.info(f"删除章节内容: {project_id} 第 {chapter} 章, 共 {count} 行")
            return count
    
    @classmethod
    def update(cls, project_id: str, chapter: int, line: int, new_content: str) -> Optional[NovelContent]:
        """
        更新指定行的内容。
        ...
        with DatabaseSession() as db:
            ...
            content = db.exec(statement).first()
            if not content:
                logger.warning(f"行内容不存在，无法更新: {project_id} 第 {chapter} 章 第 {line} 行")
                return None
            ...
            logger.info(f"更新行内容: {project_id} 第 {chapter} 章 第 {line} 行")
            return content
    
    @classmethod
    def delete_single(cls, project_id: str, chapter: int, line: int) -> bool:
        """
        删除单条内容。
        ...
        with DatabaseSession() as db:
            ...
            content = db.exec(statement).first()
            if not content:
                logger.warning(f"行内容不存在，无法删除: {project_id} 第 {chapter} 章 第 {line} 行")
                return False

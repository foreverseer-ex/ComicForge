...
async def get_actor_example_image(actor_id: str, example_index: int) -> FileResponse:
    ...
    # filename 只保存文件名，实际文件保存在 projects/{project_id}/actors/{actor_name}/{filename}
    ...
    image_path = project_home / actual_project_id / "actors" / actor.name / example.filename
    
    # 如果文件不存在，说明还在生成中或生成失败
    if not image_path.exists():
        # 如果有 job_id，可以查询 job 状态提供更详细的信息
        job_id = example.extra.get('job_id')
        if job_id:
            job = JobService.get(job_id)
            if job and job.status == 'pending':
                raise HTTPException(status_code=404, detail="示例图正在生成中")
            elif job and job.status == 'failed':
                raise HTTPException(status_code=404, detail="示例图生成失败")
        raise HTTPException(status_code=404, detail=f"示例图文件不存在: {example.filename}")

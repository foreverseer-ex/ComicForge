...
class IterationChatMessage(BaseModel):
    """迭代模式专用消息类型"""
    ...
    role: Literal["system", "user", "assistant"] = Field(
        default="assistant",
        description="消息角色，迭代消息通常是assistant"
    )
    
    messages: list[MessageContent] = Field(
        default_factory=list,
        description="消息内容列表（可能包含工具调用，通常只记录最终操作的工具调用）"
    )
    ...
    stop: int = Field(
        description="迭代终止条件。对于全文迭代，这是终止行号（通常是session.total_lines）"
    )
...
# 消息联合类型
ChatMessageType = Union[ChatMessage, IterationChatMessage]


class ChatHistory(BaseModel):
    """聊天历史记录"""
    
    messages: list[ChatMessageType] = Field(
        default_factory=list,
        description="消息列表（可以是普通消息或迭代消息）"
    )
    ...
    def add_iteration_message(self, iteration_msg: IterationChatMessage):
        """添加迭代消息到历史记录"""
        self.messages = self.messages + [iteration_msg]
    ...
    def get_messages_for_llm(self) -> list[tuple[str, str]]:
        ...
        对于 IterationChatMessage：
        - 如果迭代已完成（index >= stop），作为普通消息处理
        - 如果迭代未完成，跳过（避免混淆）

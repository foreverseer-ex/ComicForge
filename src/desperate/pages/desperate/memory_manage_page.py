...
class MemoryManagePage(ft.Column):
    ...
    def _prepare_memories_data(self):
        """准备记忆数据（不更新UI，用于初始化）"""
    ...
    def _on_memory_created(self, key: str):
        ...
        if self.page:
            async def show_toast_async():
                await asyncio.sleep(0.1)  # 短暂延迟，确保页面更新完成
                self._show_toast(f"创建记忆成功: {key}", ft.Colors.GREEN_700)
            
            self.page.run_task(show_toast_async)
...
class CreateMemoryDialog(ft.AlertDialog):
    ...
    def __init__(self, project_id: str, on_success: callable, on_error: callable):
        """
        初始化创建记忆对话框
...
class DeleteMemoryConfirmDialog(ft.AlertDialog):
    ...
    def _on_confirm(self, e):
        """确认删除"""
        try:
            MemoryService.remove(self.memory.memory_id)

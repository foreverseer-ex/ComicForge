...
class SettingsPage(ft.Column):
    ...
    def __init__(self):
        ...
        self._old_values = {
            ...
            'llm_timeout': app_settings.llm.timeout,
            'llm_summary_epoch': app_settings.llm.summary_epoch,
        }
        ...
        self.llm_timeout_field = ft.TextField(
            label="请求超时",
            value=str(app_settings.llm.timeout),
            hint_text=llm_fields['timeout'].description,
            keyboard_type=ft.KeyboardType.NUMBER,
            width=200,
            on_blur=lambda _: self._save_llm_timeout(),
            on_submit=lambda _: self._save_llm_timeout(),
        )
    ...
    def _save_default_username(self):
        ...
        self._save_config(reason="修改默认用户名")
    ...
    def _save_civitai_api_token(self):
        ...
        self._save_config(reason="修改 Civitai API Token")
    ...
    def _save_civitai_timeout(self):
        ...
        try:
            ...
            self._save_config(reason="修改 Civitai 超时时间")
    ...
    def _save_sd_forge_base_url(self):
        ...
        self._save_config(reason="修改 SD Forge Base URL")
    ...
    def _save_sd_forge_home(self):
        ...
        self._save_config(reason="修改 SD Forge 安装目录")
    ...
    def _save_sd_forge_timeout(self):
        ...
        try:
            ...
            self._save_config(reason="修改 SD Forge 超时时间")
    ...
    def _save_sd_forge_generate_timeout(self):
        ...
        try:
            ...
            self._save_config(reason="修改 SD Forge 生成超时时间")
    ...
    def _on_provider_changed(self, provider: str):
        ...
        # 更新页面
        self._save_config(reason="修改 LLM Provider")
        if self.page:
    ...
    def _on_model_changed(self, model: str):
        ...
        if model:
            ...
            self._save_config(reason="修改 LLM 模型")
    ...
    def _save_llm_model(self):
        ...
        self._save_config(reason="修改 LLM 模型名称（自定义输入）")
    ...
    def _save_llm_api_key(self):
        ...
        self._save_config(reason="修改 LLM API Key")
    ...
    def _save_llm_base_url(self):
        ...
        self._save_config(reason="修改 LLM Base URL")
    ...
    def _save_llm_temperature(self):
        ...
        try:
            ...
            self._save_config(reason="修改 LLM Temperature")
    ...
    def _save_llm_timeout(self):
        """保存 LLM 请求超时。"""
        try:
            new_value = float(self.llm_timeout_field.value)
            if new_value <= 0:
                raise ValueError("超时时间必须大于 0")
            
            app_settings.llm.timeout = new_value
            self._old_values['llm_timeout'] = new_value
            self._save_config(reason="修改 LLM 请求超时")
            logger.info(f"LLM 请求超时已更新为: {new_value} 秒")
        except ValueError as e:
            self._show_error(f"超时时间格式错误: {e}")
            self.llm_timeout_field.value = str(self._old_values['llm_timeout'])
            self.llm_timeout_field.update()
    
    def _save_llm_developer_mode(self):
        """保存 LLM 开发者模式。"""
        app_settings.llm.developer_mode = self.llm_developer_mode_switch.value
        self._save_config(reason="修改 LLM 开发者模式")
    ...
    def _save_llm_system_prompt(self):
        ...
        self._save_config(reason="修改 LLM 系统提示词")
    
    def _save_llm_summary_epoch(self):
        """保存对话总结周期配置。"""
        try:
            new_value = int(self.llm_summary_epoch_field.value)
            if new_value < 2 or new_value > 1000:
                self._show_error("对话总结周期必须在 2-1000 之间")
                self.llm_summary_epoch_field.value = str(self._old_values['llm_summary_epoch'])
                self.llm_summary_epoch_field.update()
                return
            
            app_settings.llm.summary_epoch = new_value
            self._old_values['llm_summary_epoch'] = new_value
            self._save_config(reason="修改对话总结周期")
    ...
    def _save_draw_backend(self, backend: str):
        ...
        self._save_config(reason="修改绘图后端")
    
    def _save_config(self, reason: str = "设置页面修改"):
        """保存配置到文件并显示提示。
        
        :param reason: 保存原因（用于日志记录）
        """
        if app_settings.save(reason=reason):
    ...
    def _on_reset_confirmed(self):
        ...
        import time
        import sys
        from api.services.db import drop_all_tables, init_db

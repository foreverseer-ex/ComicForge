...
from api.components.desperate.model_card import ModelCard
from api.constants.model_meta import Ecosystem, BaseModel
from api.schemas.model_meta import ModelMeta
from api.services.model_meta import local_model_meta_service, civitai_model_meta_service
...
class ModelManagePage(ft.Column):
    ...
    def __init__(self, page: ft.Page = None):
        ...
        for attr_name in dir(Ecosystem):
            if not attr_name.startswith('_'):  # 跳过私有属性
                ...
                if isinstance(attr_value, str):  # 只获取字符串常量
                    # 直接使用原始值作为显示名称
                    ecosystem_options.append(ft.dropdown.Option(key=attr_value, text=attr_value))
        
        # 清理配置：将 None、"None" 或 "所有" 转换为空字符串（表示"所有"）
        # 处理可能的异常值，确保统一使用空字符串表示"所有"
        if (app_settings.ui.ecosystem_filter is None or 
            app_settings.ui.ecosystem_filter == "None" or 
            app_settings.ui.ecosystem_filter == "所有"):
            app_settings.ui.ecosystem_filter = ""
        # 清理配置：将 None、"None" 或 "所有" 转换为空字符串（表示"所有"）
        if (app_settings.ui.base_model_filter is None or 
            app_settings.ui.base_model_filter == "None" or 
            app_settings.ui.base_model_filter == "所有"):
            app_settings.ui.base_model_filter = ""
        
        # 注意：清除筛选按钮的初始化在创建完下拉框之后
        
        # 初始化日志
        logger.debug(f"初始化筛选器: ecosystem_filter={app_settings.ui.ecosystem_filter}, base_model_filter={app_settings.ui.base_model_filter}")
        
        self.ecosystem_filter_dropdown = ft.Dropdown(
            label="生态系统",
            options=ecosystem_options,
            value=app_settings.ui.ecosystem_filter if app_settings.ui.ecosystem_filter else None,
            on_change=self._on_ecosystem_filter_change,
            expand=True,
        )
        
        # 基础模型筛选下拉菜单 - 动态生成选项（移除"所有"选项）
        base_model_options = []
        ...
        for attr_name in dir(BaseModel):
            if not attr_name.startswith('_'):  # 跳过私有属性
                ...
                if isinstance(attr_value, str):  # 只获取字符串常量
                    # 直接使用原始值作为显示名称
                    base_model_options.append(ft.dropdown.Option(key=attr_value, text=attr_value))
        
        self.base_model_filter_dropdown = ft.Dropdown(
            label="基础模型",
            options=base_model_options,
            value=app_settings.ui.base_model_filter if app_settings.ui.base_model_filter else None,
        ...
        self.clear_filter_button = ft.IconButton(
            ...
            on_click=self._on_clear_filter,
            visible=bool(app_settings.ui.ecosystem_filter or app_settings.ui.base_model_filter),
        )
        
        # Checkpoint 区域（占位，后面会动态填充）
        self.checkpoint_section = ft.Column(spacing=self.responsive.spacing_medium)
        
        # LoRA 区域（占位，后面会动态填充）
        self.lora_section = ft.Column(spacing=self.responsive.spacing_medium)
        ...
        self.open_all_urls_button = ft.IconButton(
            icon=ft.Icons.OPEN_IN_BROWSER,
            tooltip="在浏览器中打开所有模型的网页链接",
            icon_color=ft.Colors.GREEN_400,
            on_click=self._on_open_all_urls
        )
        
        # 组合布局：两个筛选菜单 + 清除筛选按钮 + 隐私模式按钮 + 其他按钮 + Checkpoint + 分隔线 + LoRA
        self.controls = [
            ft.Container(
                content=ft.Row(
                    controls=[
                ...
                padding=ft.padding.only(top=self.responsive.spacing_small, bottom=self.responsive.spacing_medium),
            ),
        ...
        self.spacing = self.responsive.spacing_medium
    ...
    def _on_ecosystem_filter_change(self, e: ft.ControlEvent):
        ...
        # 调试日志
        logger.info(f"生态系统筛选改变: selected_value={selected_value!r}, type={type(selected_value).__name__}")
        
        # None 表示未选择（显示所有）
        if selected_value is None:
            app_settings.ui.ecosystem_filter = ""
        else:
            app_settings.ui.ecosystem_filter = selected_value
        
        logger.info(f"设置 ecosystem_filter = {app_settings.ui.ecosystem_filter!r}")
    ...
    def _on_clear_filter(self, e: ft.ControlEvent):
        """清除筛选条件"""
        logger.info("清除筛选条件")
        ...
        # 重置下拉框
        self.ecosystem_filter_dropdown.value = None
        self.base_model_filter_dropdown.value = None
        
        # 更新下拉框UI
        if self.page:
            self.ecosystem_filter_dropdown.update()
            self.base_model_filter_dropdown.update()
    ...
    def _update_clear_filter_button_visibility(self):
        """更新清除筛选按钮的可见性"""
        has_filter = bool(app_settings.ui.ecosystem_filter or app_settings.ui.base_model_filter)
        self.clear_filter_button.visible = has_filter
        if self.page:
            self.clear_filter_button.update()
    
    def _on_base_model_filter_change(self, e: ft.ControlEvent):
        """基础模型筛选条件改变时的回调。"""
        selected_value = e.control.value
        
        # 调试日志
        logger.info(f"基础模型筛选改变: selected_value={selected_value!r}, type={type(selected_value).__name__}")
        ...
        logger.info(f"设置 base_model_filter = {app_settings.ui.base_model_filter!r}")
    ...
    def _filter_models(self, models):
        ...
        # 按生态系统筛选（空字符串、None 和字符串 "None" 都表示不筛选）
        ecosystem_filter = app_settings.ui.ecosystem_filter
        logger.info(f"筛选模型: 原始数量={len(models)}, ecosystem_filter={ecosystem_filter!r}, type={type(ecosystem_filter).__name__}")
        
        # 只有当筛选值是非空字符串时才进行筛选
        if ecosystem_filter and ecosystem_filter != "None":
            before_count = len(filtered)
            filtered = [m for m in filtered if m.ecosystem == ecosystem_filter]
            logger.info(f"生态系统筛选后: {before_count} -> {len(filtered)}")
        else:
            logger.info(f"生态系统筛选: 跳过（值为 {ecosystem_filter!r}，显示所有）")
        
        # 按基础模型筛选（空字符串、None 和字符串 "None" 都表示不筛选）
        base_model_filter = app_settings.ui.base_model_filter
        logger.info(f"base_model_filter={base_model_filter!r}, type={type(base_model_filter).__name__}")
        
        # 只有当筛选值是非空字符串时才进行筛选
        if base_model_filter and base_model_filter != "None":
            before_count = len(filtered)
            # 注意：base_model 可能为 None，需要处理
            filtered = [m for m in filtered if m.base_model == base_model_filter]
            logger.info(f"基础模型筛选后: {before_count} -> {len(filtered)}")
        else:
            logger.info(f"基础模型筛选: 跳过（值为 {base_model_filter!r}，显示所有）")
        
        logger.info(f"最终筛选结果: {len(filtered)} 个模型")
        return filtered
    
    def _on_open_all_urls(self, _: ft.ControlEvent):
        """一键打开所有模型的网页链接（按 model_id 去重）"""
        try:
            # 获取所有模型（包括 Checkpoint 和 LoRA）
            all_checkpoint_models = local_model_meta_service.sd_list
            all_lora_models = local_model_meta_service.lora_list
            all_models = all_checkpoint_models + all_lora_models
            
            # 按 model_id 去重收集网页链接（每个 model_id 只保留一个 URL）
            model_id_to_url = {}
            for model in all_models:
                if model.web_page_url and model.model_id not in model_id_to_url:
                    model_id_to_url[model.model_id] = model.web_page_url
            
            urls = list(model_id_to_url.values())
            
            if not urls:
                self._show_toast("❌ 没有找到任何模型的网页链接", ft.Colors.ORANGE_700)
                return
            ...
            for url in urls:
                try:
                    ...
                    logger.warning(f"打开URL失败: {url}, 错误: {e}")
            
            total_models = len(all_models)
            unique_models = len(urls)
            self._show_toast(
                f"✅ 已在浏览器中打开 {opened_count}/{unique_models} 个模型链接（共 {total_models} 个模型，已去重）", 
                ft.Colors.GREEN_700
            )
            logger.info(f"已打开 {opened_count}/{unique_models} 个模型链接（共 {total_models} 个模型，已按 model_id 去重）")
            
        except Exception as e:
            logger.exception(f"打开所有模型URL失败: {e}")
            self._show_toast(f"❌ 打开失败: {str(e)}", ft.Colors.RED_700)
    ...
    def _render_models(self):
        ...
        checkpoint_flow = ft.Row(
            ...
            wrap=True,
            run_spacing=self.responsive.spacing_small,
        ...
        # 更新 Checkpoint 区域
        self.checkpoint_section.controls = [
            ft.Text("Checkpoint", size=self.responsive.font_title, weight=ft.FontWeight.BOLD),
            checkpoint_flow,
        ]
        ...
        # 更新 LoRA 区域
        self.lora_section.controls = [
            ft.Text("LoRA", size=self.responsive.font_title, weight=ft.FontWeight.BOLD),
            lora_flow,
        ]

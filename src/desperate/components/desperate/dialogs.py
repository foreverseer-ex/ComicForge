...
class CreateProjectDialog(ft.AlertDialog):
    """
    创建项目对话框。
    
    用法：
        dialog = CreateProjectDialog(page, on_success=callback)
        page.open(dialog)
    """
    ...
    def __init__(self, page: ft.Page, on_success=None):
        ...
        super().__init__(
            modal=True,
            title=ft.Text("创建项目", size=20, weight=ft.FontWeight.BOLD),
    ...
    def _on_create(self, _e):
        ...
        # 验证
        if not title:
            self._show_toast("请输入项目标题或选择文件", ft.Colors.RED_700)
            return
        ...
        try:
            ...
            created = ProjectService.create(new_project)
            logger.info(f"创建项目成功: {created.project_id}, 项目路径: {created.project_path}")
            ...
            self._show_toast(f"创建项目成功: {title}", ft.Colors.GREEN_700)
            
        except Exception as ex:  # pylint: disable=broad-except
            logger.exception(f"创建项目失败: {ex}")
            self._show_toast(f"创建项目失败: {ex}", ft.Colors.RED_700)
...
class DeleteProjectDialog(ft.AlertDialog):
    """
    删除项目确认对话框。
    
    用法：
        dialog = DeleteProjectDialog(page, project, on_success=callback)
        page.open(dialog)
    """
    
    def __init__(self, page: ft.Page, project: Project, on_success=None):
        """
        初始化删除项目对话框。
    ...
    def _on_delete(self, _e):
        """删除按钮点击"""
        try:
            success = ProjectService.delete(self.project.project_id)
            if success:
                logger.info(f"删除项目: {self.project.project_id}")
                ...
                self.page.update()
                time.sleep(0.1)  # 添加小延迟以避免白屏问题
                self._show_toast("删除项目失败", ft.Colors.RED_700)
                    
        except Exception as ex:  # pylint: disable=broad-except
            logger.exception(f"删除项目失败: {ex}")
            self.open = False
            self.page.update()
            time.sleep(0.1)  # 添加小延迟以避免白屏问题
            self._show_toast(f"删除项目失败: {ex}", ft.Colors.RED_700)

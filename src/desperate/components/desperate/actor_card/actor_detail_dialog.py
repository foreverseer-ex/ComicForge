...
class ActorDetailDialog(ft.AlertDialog):
    ...
    def __init__(self, actor: Actor, all_actors: list[Actor] = None, current_index: int = 0):
        ...
        # 构建标题栏：标题 + 右侧按钮（生成立绘/重新生成 + 关闭）
        self.title_text = ft.Text("Actor 详情", size=18, weight=ft.FontWeight.BOLD)
        
        # 检查是否有立绘（示例图）
        has_portrait = bool(actor.examples and len(actor.examples) > 0)
        
        # 生成立绘/重新生成按钮
        self.generate_button = ft.IconButton(
            icon=ft.Icons.PHOTO_CAMERA if not has_portrait else ft.Icons.REFRESH,
            tooltip="重新生成立绘" if has_portrait else "生成立绘",
            on_click=self._open_generate_dialog,
        )
    ...
    def _build_info_rows(self) -> list[ft.Row]:
        ...
        rows = [
            ...
            _make_row("Session ID", actor.project_id),
        ]
    ...
    def _update_content(self):
        ...
        # 更新生成立绘按钮图标（根据是否有立绘）
        has_portrait = bool(self.actor.examples and len(self.actor.examples) > 0)
        self.generate_button.icon = ft.Icons.PHOTO_CAMERA if not has_portrait else ft.Icons.REFRESH
        self.generate_button.tooltip = "重新生成立绘" if has_portrait else "生成立绘"
    ...
    def _open_generate_dialog(self, e: ft.ControlEvent):
        """打开生成立绘对话框"""
        page = e.page if e else self.page
        if not page:
            logger.error("page 为 None，无法打开对话框")
            return
        
        try:
            from api.components.desperate.actor_card.generate_portrait_dialog import GeneratePortraitDialog
            
            # 创建生成立绘对话框
            dialog = GeneratePortraitDialog(
                actor_name=self.actor.name,
                project_id=self.actor.project_id,
                actor_id=self.actor.actor_id,
                on_generate=lambda **kwargs: page.run_task(self._do_generate_portrait, **kwargs),
                on_error=self._show_error,
            )
            
            # 打开对话框
            page.open(dialog)
            logger.info("生成立绘对话框已打开")
        except Exception as ex:
            logger.exception(f"打开生成立绘对话框失败: {ex}")
            self._show_error(f"打开对话框失败: {str(ex)}")
    
    async def _do_generate_portrait(
        self,
        title: str,
        desc: str,
        model: str,
        ...
        try:
            # 调用 API
            base_url = app_settings.mcp.base_url
            url = f"{base_url}/actor/{self.actor.actor_id}/add_portrait"
            ...
            logger.success(f"生成立绘成功: {title}")
            
            # 显示成功消息
            self._show_success(f"✅ 生成立绘成功: {title}")
            
            # 重新加载 Actor 数据（通过重新打开对话框实现）
            # 先关闭当前对话框
            if self.page:
            ...
            updated_actor = ActorService.get(self.actor.actor_id)
            if updated_actor:
                # 重新打开对话框显示更新后的信息
                from api.components.desperate.actor_card.actor_detail_dialog import ActorDetailDialog
                new_dialog = ActorDetailDialog(
                    actor=updated_actor,
                    all_actors=self.all_actors,
                    current_index=self.current_index,
                )
                if self.page:
                    self.page.open(new_dialog)
        except Exception as ex:
            logger.exception(f"生成立绘失败: {ex}")
            self._show_error(f"❌ 生成立绘失败: {str(ex)}")

...
AI 驱动的漫画创作与可视化工具：对话、绘图、模型元数据，一体化前后端分离架构。
...
- **模型喜爱**：支持标记喜爱的模型，快速筛选常用模型
- **隐私模式**：支持隐私模式，隐藏模型预览图（适用于公共场合）

### 👥 角色（Actor）管理
- **角色创建**：支持创建角色、地点、组织等实体
- **标签系统**：预定义标签（外观、服装、性格、背景等）
- **示例图管理**：为角色添加多张示例图，支持添加、删除示例图
...
- **Civitai 导入**：通过 AIR 标识符导入模型元数据
  - **批量导入**：支持批量导入多个模型（AIR 标识符列表）
  - **并发控制**：可配置最大并发数和并行工作线程数
  - **超时设置**：可配置绘图任务超时时间（60-3600秒）
- **筛选功能**：按生态系统和基础模型筛选
- **模型喜爱**：支持标记喜爱的模型，快速筛选常用模型
